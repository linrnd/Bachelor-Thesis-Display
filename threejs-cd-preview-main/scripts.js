...
function LoadTextures(textureLoader, textureIndex, textureCount) {
    textureLoader.load(
        ...
        function ( texture ) {
            // --- safer texture setup for blurry PNGs ---
            texture.colorSpace = THREE.SRGBColorSpace;

            // keep the glow edges clean (no repeated seam)
           // mirror horizontally without turning upside-down
texture.rotation = 0;

texture.wrapS = THREE.RepeatWrapping;      // allow mirroring on X
texture.wrapT = THREE.ClampToEdgeWrapping; // clamp on Y
texture.repeat.set(-1, 1);                 // flip horizontally only
texture.offset.set(1, 0);                  // keep texture aligned


            // better handling of soft transparency
